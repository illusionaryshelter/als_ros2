...
private:
  ...
  inline double bin2val(int bin) { return (double)bin * binWidth_ + minValue_; }
...
public:
  ...
  Histogram(std::vector<double> values, double binWidth, double minValue,
            double maxValue)
      : binWidth_(binWidth), minValue_(minValue), maxValue_(maxValue) {
    buildHistogram(values);
  }
