...
#error "this file is reserved for SLAMER"
...
public:
  ISM(void) : ismPoints_(new pcl::PointCloud<pcl::PointXYZRGB>()) {}
  ...
  void readISM(std::string ismYamlFile) {
    ...
    std::string dirPath =
        ismYamlPath.substr(0, ismYamlPath.find_last_of("/") + 1);
    ...
    if (ismImage.empty()) {
      RCUTILS_LOG_ERROR("Could not read the ISM image -> %s",
                        ismFilePath.c_str());
      exit(1);
    }
    ...
    for (int i = 0; i < (int)objectNames_.size(); ++i) {
      ...
      // std::cout << i << " " << objectColors_[i][0] << " " <<
      // objectColors_[i][1] << " " << objectColors_[i][2] << std::endl;
    }
    ...
    for (int u = 0; u < ismImage.cols; ++u) {
      for (int v = 0; v < ismImage.rows; ++v) {
        ...
        for (int i = 0; i < (int)objectNames_.size(); ++i) {
          if (val[0] == objectColors_[i][2] && val[1] == objectColors_[i][1] &&
              val[2] == objectColors_[i][0]) {
            pcl::PointXYZRGB p;
            ...
            // printf("%s, at (u, v) = (%d, %d) with (r, g, b) = (%d, %d,
            // %d)\n",
