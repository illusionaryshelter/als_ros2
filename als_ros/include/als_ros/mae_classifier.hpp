...
public:
  ...
  void
  learnThreshold(std::vector<std::vector<double>> trainSuccessResidualErrors,
    ...
    bool isFirst = true;
    double maxAcc;
    int totalNum = (int)trainSuccessResidualErrors.size() +
                   (int)trainFailureResidualErrors.size();
  ...
  void writeClassifierParams(
    ...
    __attribute__((unused)) int retVal = system(mkdirCmd.c_str());
    ...
    std::string truePositiveMAEsFileName =
        classifiersDir_ + "true_positive_maes.txt";
    std::string trueNegativeMAEsFileName =
        classifiersDir_ + "true_negative_maes.txt";
    std::string falsePositiveMAEsFileName =
        classifiersDir_ + "false_positive_maes.txt";
    std::string falseNegativeMAEsFileName =
        classifiersDir_ + "false_negative_maes.txt";
    ...
    truePositiveNum_ = falseNegativeNum_ = falsePositiveNum_ =
        trueNegativeNum_ = 0;
  ...
  void readClassifierParams(void) {
    ...
    std::string positiveMAEsFilePath =
        classifiersDir_ + lconf["positiveMAEsFileName"].as<std::string>();
    std::string negativeMAEsFilePath =
        classifiersDir_ + lconf["negativeMAEsFileName"].as<std::string>();
    std::string truePositiveMAEsFilePath =
        classifiersDir_ + lconf["truePositiveMAEsFileName"].as<std::string>();
    std::string trueNegativeMAEsFilePath =
        classifiersDir_ + lconf["trueNegativeMAEsFileName"].as<std::string>();
    std::string falsePositiveMAEsFilePath =
        classifiersDir_ + lconf["falsePositiveMAEsFileName"].as<std::string>();
    std::string falseNegativeMAEsFilePath =
        classifiersDir_ + lconf["falseNegativeMAEsFileName"].as<std::string>();
    positiveMAEHistogram_ =
        Histogram(readMAEs(positiveMAEsFilePath), maeHistogramBinWidth_);
    negativeMAEHistogram_ =
        Histogram(readMAEs(negativeMAEsFilePath), maeHistogramBinWidth_);
    truePositiveMAEHistogram_ =
        Histogram(readMAEs(truePositiveMAEsFilePath), maeHistogramBinWidth_);
    trueNegativeMAEHistogram_ =
        Histogram(readMAEs(trueNegativeMAEsFilePath), maeHistogramBinWidth_);
    falsePositiveMAEHistogram_ =
        Histogram(readMAEs(falsePositiveMAEsFilePath), maeHistogramBinWidth_);
    falseNegativeMAEHistogram_ =
        Histogram(readMAEs(falseNegativeMAEsFilePath), maeHistogramBinWidth_);

    positiveMAEHistogram_.smooth();
    negativeMAEHistogram_.smooth();
    truePositiveMAEHistogram_.smooth();
    trueNegativeMAEHistogram_.smooth();
    falsePositiveMAEHistogram_.smooth();
    falseNegativeMAEHistogram_.smooth();
    ...
    dTruePositive_ =
        (double)truePositiveNum / (double)(truePositiveNum + falseNegativeNum);
    dFalsePositive_ =
        (double)falsePositiveNum / (double)(truePositiveNum + falseNegativeNum);
    dTrueNegative_ =
        (double)trueNegativeNum / (double)(trueNegativeNum + falsePositiveNum);
    dFalseNegative_ =
        (double)falseNegativeNum / (double)(trueNegativeNum + falsePositiveNum);
  }
  ...
  void writeDecisionLikelihoods(void) {
    ...
    for (double rel = 0.0; rel <= 1.0 + 0.05; rel += 0.05) {
      for (double mae = 0.0; mae <= 0.7 + maeHistogramBinWidth_;
           mae += maeHistogramBinWidth_) {
        double pSuccessPositive =
            dTruePositive_ * truePositiveMAEHistogram_.getProbability(mae);
        double pFailurePositive =
            dFalsePositive_ * falsePositiveMAEHistogram_.getProbability(mae);
        ...
        double pSuccessNegative =
            dFalseNegative_ * falseNegativeMAEHistogram_.getProbability(mae);
        double pFailureNegative =
            dTrueNegative_ * trueNegativeMAEHistogram_.getProbability(mae);
